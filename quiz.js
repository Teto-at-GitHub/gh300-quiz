...
function shuffleQuestion(q) {
  const indexed = q.opts.map((opt, i) => ({ opt, wasCorrect: q.correct.includes(i) }));
  indexed.sort(() => Math.random() - 0.5);
  return {
    ...q,
    opts: indexed.map(x => x.opt),
    correct: indexed.map((x, i) => x.wasCorrect ? i : -1).filter(i => i !== -1),
  };
}
...
function init() {
  pool = sample(ALL_QUESTIONS, EXAM_SIZE).map(shuffleQuestion);
  current = 0;
  selected = [];
  submitted = false;
  results = [];
...
function renderQ() {
  ...
  document.getElementById('root').innerHTML = `
    ...
    <div class="question-card">
      <p class="question-text">${esc(q.q)}</p>
  ...
  q.opts.forEach((o, i) => {
    ...
    b.className = 'opt';
    b.id = 'opt' + i;
    b.innerHTML = `<span class="opt-key">${KEYS[i]}</span><span>${esc(o)}</span>`;
...
function submit() {
  ...
  document.getElementById('expbox').innerHTML =
    `<div class="explanation"><strong>${isOk ? '✓ Correct' : '✗ Incorrect'}</strong> — ${esc(q.exp)}</div>`;
